                ...
                <div class="modal fade" id="modalAssigned{{ $task->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <form method="POST" action="{{ route('tasks.submit') }}" enctype="multipart/form-data">
                                @csrf
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="form-group">
                                        <label for="submitted_file">Upload Attachment</label>
                                        <input type="file" class="form-control" id="submitted_file" name="submitted_file">
                                    </div>
        ...
        <table class="table table-striped">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ \Carbon\Carbon::parse($task->due_date)->format('F j, Y') }}</td>
                    <td>{{ $task->completed }}</td>
                    <td>{{ $task->task_response }}</td>
                    <td>
                        @if($task->files)
                            <a href="{{ $task->submitted_file }}" target="_blank">Download</a>
                        @else
                            No File
                        @endif
                    </td>
                ...
                <div class="modal fade" id="modalAssigned{{ $task->id }}" tabindex="-1" role="dialog" aria-labelledby="modalLabel{{ $task->id }}" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="modalLabel{{ $task->id }}">Task Details</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    <textarea class="form-control" name="task_response" id="task_response" readonly>{!! $task->task_response !!}</textarea>
                                </div>
                                <hr>
                                @if(!empty($task->getMedia('attachments')))
                                    <h5>Attachments</h5>
                                    <ul>
                                        @if($task->files)
                                            <a href="{{ $task->submitted_file }}" target="_blank">Download</a>
...
<!-- <script>
                        ...
                        return loader.file
                            .then(function(file) {
                                return new Promise(function(resolve, reject) {
                                    ...
                                    xhr.setRequestHeader('x-csrf-token', window._token); // Ensure window._token is set
                                    ...
                                    data.append('crud_id', '{{ $assignTask->id ?? 0 }}'); // Ensure $assignTask->id is available
                                    xhr.send(data);
                                });
                            });
                    }
    ...
    Dropzone.options.attachmentsDropzone = {
        url: '{{ route('admin.assign-tasks.storeMedia') }}', // Ensure route is correct
        maxFilesize: 2, // MB
        addRemoveLinks: true,
        headers: {
        ...
        removedfile: function(file) {
            ...
            var name = file.file_name || uploadedAttachmentsMap[file.name] || '';
        ...
        error: function(file, response) {
            ...
            errorMessages.forEach(function(node) {
