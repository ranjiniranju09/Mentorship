            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="custom-card modules">
                        <a href="{{route('menteemodule.index')}}"><i class=" icon fa-solid fa-diagram-project fa-2xl"></i></a>
                                </br>
                        <h5>Modules</h5>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if(isset($sessions[$module->id]) && count($sessions[$module->id]) > 0)
                                        @foreach($sessions[$module->id] as $session)
                                            @php
                                                $sessionTimeOver = \Carbon\Carbon::parse($session->sessiondatetime)->isPast();
                                            @endphp

                                            @if($sessionTimeOver)
                                                <span class="badge badge-success">Completed</span>
                                            @elseif(!empty($session->sessiondatetime))
                                ...
                                <td>
                                    @if(isset($sessions[$module->id]) && count($sessions[$module->id]) > 0)
                                        @foreach($sessions[$module->id] as $session)
                                            <a href="{{ $sessionTimeOver ? 'javascript:void(0);' : $session->sessionlink }}" 
                                            class="btn btn-xs {{ $sessionTimeOver ? 'btn-secondary disabled' : 'btn-primary' }}" 
                                            target="{{ $sessionTimeOver ? '_self' : '_blank' }}">
                                                Join Session
                                            </a>
                                            @if($session->done == 1 && !empty($session->file_path))
                                                <a href="{{ ($session->file_path) }}" class="btn btn-info btn-sm" target="_blank">
