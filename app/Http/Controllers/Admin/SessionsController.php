...
class SessionsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.sessionprogress.index', compact('modules', 'mentors', 'mentees'));
    }

    public function sessionprogress()
{
    // Assuming admin access, remove mentor-specific filtering
    // Total Sessions
    ...
    $totalGuestSessions = DB::table('guest_lectures')
        ->whereNotNull('id') // Assuming guest sessions are identified by a non-null 'guest_id'
        ->count();

    // Past Guest Sessions
    $pastGuestSessions = DB::table('guest_lectures')
        ->whereNotNull('id')
        ->where('guestsession_date_time', '<', now())
        ->count();

    // Upcoming Guest Sessions
    $upcomingGuestSessions = DB::table('guest_lectures')
        ->whereNotNull('id')
        ->where('guestsession_date_time', '>=', now())
        ->count();

    // Pass the data to the view
    return view('admin.sessionprogress.index', [
        'totalSessions' => $totalSessions,
        'pastSessions' => $pastSessions,
        'upcomingSessions' => $upcomingSessions,
        'totalGuestSessions' => $totalGuestSessions,
        'pastGuestSessions' => $pastGuestSessions,
