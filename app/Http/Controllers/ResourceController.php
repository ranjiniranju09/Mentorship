...
//         $resource->title = $request->input('editResourceName');
//         $resource->module_id = $request->input('module_id');
//         $resource->description = $request->input('editdescription');
//         $resource->file_path = $request->input('file_path');
//         $resource->type = $request->input('edittype');
...
class ResourceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // if ($resource->type === 'normal' || ($resource->type === 'private' && $request->visibility === 'all')) {
        //     ResourceApproval::create([
        //         'resource_id' => $resource->id,
        //         'admin_id' => null,
        //         'is_approved' => false,
        ...
        $resources = DB::table('resources')->get(); // Fetch updated resources
        return view('resources.index', compact('resources', 'modules'))->with('success', 'Resource added successfully');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $resources = DB::table('resources')->get(); // Fetch updated resources
        $modules = DB::table('modules')->get(); // Fetch updated modules

        return view('resources.index', compact('resources', 'modules'))->with('success', 'Resource updated successfully');
    }
